...
async fn make_anthropic_api_call(api_key: &str) -> Result<(f64, String), Box<dyn std::error::Error>> {
    let client = reqwest::Client::new();
    let start = Instant::now();
...
async fn test_scenario_with_real_apis(scenario: &TestScenario, iterations: usize) -> ApiTestResult {
    println!("🧪 Testing: {} ({} real API calls)", scenario.name, iterations);
                ...
                match api_result {
                    Ok((total_latency, provider)) => {
                        ...
                        *provider_distribution.entry(provider).or_insert(0) += 1;
                        println!("   ✅ Request {}: {:.2}ms total, {:.0}ns routing ({})", 
                                i + 1, total_latency, routing_time, route.provider);
                    },
                    Err(e) => {
                        failed += 1;
    ...
    println!("   🔀 Avg Routing Latency: {:.0}ns ({:.6}ms)", avg_routing_latency, avg_routing_latency / 1_000_000.0);
    println!("   📈 Routing Overhead: {:.4}% of total time", routing_overhead_percent);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for result in &all_results {
        ...
        println!("   Successful: {} ({:.1}%)", result.successful_requests, result.success_rate);
        println!("   Failed: {}", result.failed_requests);
        
        if result.successful_requests > 0 {
            println!("   Avg Total Latency: {:.2}ms", result.avg_total_latency_ms);
            println!("   Avg Routing Latency: {:.0}ns ({:.6}ms)", 
            ...
            println!("   Routing Overhead: {:.4}% of total request time", result.routing_overhead_percent);
            println!("   Provider Distribution: {:?}", result.provider_distribution);
        }
    }
    ...
    for result in &all_results {
        if result.successful_requests > 0 {
            println!("{}: {:.2}ms total ({:.0}ns routing = {:.4}% overhead)", 
                result.scenario_name,
